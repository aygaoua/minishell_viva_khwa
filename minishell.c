...
/*   Updated: 2023/11/07 14:50:44 by azgaoua          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_nbr_of_elem(char *s)
{
    ...
    while (s[i])
    {
        if ((s[i] == '<' && s[i + 1] == '<') || (s[i] == '>' && s[i + 1] == '>') || (s[i] == '<' && s[i + 1] && s[i + 1] != '<') || (s[i] == '>' && s[i + 1] && s[i + 1] != '>') || s[i] == '|')
            len++;
        i++;
    }
    return (len);
...
char    *ft_convert_line(t_tokens **cmdline)
{
    ...
    while ((*cmdline)->input[i])
    {
        if (s[j] != ' ')
            ...
            s[++j] = (*cmdline)->input[i];
        if ( ((*cmdline)->input[i] == '<' && (*cmdline)->input[i + 1] == '<') || ((*cmdline)->input[i] == '>' && (*cmdline)->input[i + 1] == '>'))
        {
            j++;
            i++;
            s[j] = (*cmdline)->input[i];
        }
        if (((*cmdline)->input[i] && (*cmdline)->input[i + 1] == '>') || ((*cmdline)->input[i] && (*cmdline)->input[i + 1] == '<') || ((*cmdline)->input[i] == '<' && (*cmdline)->input[i + 1] && (*cmdline)->input[i + 1] != '<') || ((*cmdline)->input[i] == '>' && (*cmdline)->input[i + 1] && (*cmdline)->input[i + 1] != '>') || (*cmdline)->input[i] == '|')
        {
            j++;
            s[j] = ' ';
        }
