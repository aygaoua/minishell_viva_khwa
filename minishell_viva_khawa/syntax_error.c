...
/*   Updated: 2023/11/28 11:33:37 by azgaoua          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_check_syntax_error(t_token *lst)
{
    ...
    while(lst)
    {
        ...
        else if(lst->type == D_QUOT)
        {
            ...
            && ((!lst->next) || ((lst->next) && (lst->next->type != WORD && lst->next->type != DOLLAR) && !q)))
            return 1;
        else if((lst->type == PIP) && ((!lst->next || !lst->prev) || ((lst->next && lst->prev) && ((lst->next->type != WORD && lst->next->type != DOLLAR) || (lst->prev->type != WORD && lst->prev->type != DOLLAR)) && !q)))
            return 1;
        lst = lst->next;
    }
    ft_print_token(lst);
